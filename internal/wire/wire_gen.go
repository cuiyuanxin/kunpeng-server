...
func InitializeApp(cfg *config.Config, db *gorm.DB) *App {
	...
	roleService := ProvideRoleService(db)
	permissionService := ProvidePermissionService(db)
	departmentService := ProvideDepartmentService(db)
	fileService := ProvideFileService(cfg, db)
	statService := ProvideStatService(db)
	userController := ProvideUserController(userService, jwtManager)
	adminController := ProvideAdminController(userService, roleService, permissionService, departmentService, fileService, statService)
	authMiddleware := ProvideAuthMiddleware(jwtManager, db)
	casbinMiddleware := ProvideCasbinMiddleware(permissionService)
	routeRegistry := ProvideRouteRegistry(engine, userController, adminController, authMiddleware, casbinMiddleware)
	app := ProvideApp(cfg, db, engine, jwtManager, userService, roleService, permissionService, departmentService, fileService, statService, userController, adminController, authMiddleware, casbinMiddleware, routeRegistry)
	return app
}
